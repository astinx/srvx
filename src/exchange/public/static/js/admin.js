...
layui.define(['jquery', 'form', 'layer', 'element'], function(exports) {
	...
	$('.container .left_open i').click(function(event) {
		if($('.left-nav').css('left') == '0px') {
			...
			if($(window).width() < 768) {
				$('.page-content').animate({ left: '0', }, { step: function () { $(this).css({'transform': 'translate3d(0, 0, 0)', '-webkit-transform': 'translate3d(0, 0, 0)'}); } }, 100);
			} else {
			...
			if($(window).width() < 768) {
				...
				$('.page-content').animate({
					...
					step: function () {
						$(this).css({'transform': 'translate3d(221px, 0, 0)', '-webkit-transform': 'translate3d(221px, 0, 0)'});
					}
				}, 100);
			} else {
				$('.page-content').animate({
	...
	$('.left-nav #nav li').click(function(event) {
		if($(this).children('.sub-menu').length) {
			...
			for(var i = 0; i < $('.xframe').length; i++) {
				if($('.xframe').eq(i).attr('tab-id') == index + 1) {
	...
	// logo link iframe open 
	$('.logo').click(function (e) {
		e.preventDefault();	
		var url = $(this).children('a').attr('_href');
		var title = $(this).children('a').attr('lay-title');
		if($('#tabName>li').eq(0).attr('lay-id') == 1){
			tab.tabChange(1);
			return false;
		}else{
		...
		tabAdd: function(title, url, id) {
			...
			if(li > 0) {
				...
				element.tabAdd('wenav_tab', {
					title: title,
					content: '<iframe tab-id="' + id + '" frameborder="0" src="' + url + '" scrolling="yes" class="xframe"></iframe>',
					id: id
				});
				//当前窗口内容
	...
	/*
	    ...
	    h       弹出层高度（缺省调默认值）
	*/
	window.xShow = function(title, url, w, h) {
		if(title == null || title == '') {
			title = false;
		}
		if(url == null || url == '') {
	...
	/*弹出层+传递ID参数*/
	window.xEdit = function(title, url, id, w, h) {
		if(title == null || title == '') {
			title = false;
		}
		if(url == null || url == '') {
	...
	function setStorageCurMenu() {
		...
		text = text.split('ဆ')[0];
		var url = $('.layui-tab-content').find('.layui-show').find('.xframe').attr('src');
